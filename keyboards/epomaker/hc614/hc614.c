    ...
    switch (keycode) {
        ...
        case MO(_MFL): {
            if (!record->event.pressed) {
                ...
                Mode_Query = false;
                hs_rgb_tog_flag = false;
                if (!confinfo.hs_rgb_tog) {
                    rgb_matrix_disable();
...
bool process_record_kb(uint16_t keycode, keyrecord_t *record) {
    ...
    switch (keycode) {
         case KC_F1:{
            if(confinfo.ntf){
                if (keymap_is_mac_system()) {
                    ...
                    if (record->event.pressed) {
                        register_code16(KC_MYCM);
                    } else {
                        unregister_code16(KC_MYCM);
                    }
                    return false;
                }
            }
            else if(keymap_is_mac_system()){
                if (record->event.pressed) {
        ...
        case KC_F7:{
            if(confinfo.ntf || keymap_is_mac_system()){
                    if (!(confinfo.ntf && keymap_is_mac_system())){
                        if (record->event.pressed) {
        ...
        case RGB_TOG: {
            if (record->event.pressed) {
                confinfo.hs_rgb_tog = !confinfo.hs_rgb_tog;
                if (confinfo.hs_rgb_tog) {
                    hs_rgb_tog_flag = false;
                    writePin(LED_POWER_EN_PIN, RGB_DRIVER_EN_STATE);
                    rgb_matrix_enable();
...
bool rgb_matrix_indicators_advanced_kb(uint8_t led_min, uint8_t led_max) {
    ...
    if (!charge_flag){
        ...
        uint8_t r,g,b;
        r = rgb_matrix_ws2812_array[61].r;
        g = rgb_matrix_ws2812_array[61].g;
        b = rgb_matrix_ws2812_array[61].b;
        rgb_matrix_set_color(53,r,g,b);
    }
    if (confinfo.ntf && (start_hsv.v != 0 || Mode_Query)) rgb_matrix_set_color(60,0xff,0xff,0xff);
    if (Mode_Query) {
        if (hs_rgb_tog_flag) {
            rgb_matrix_set_color_all(0x00, 0x00, 0x00);
        }
