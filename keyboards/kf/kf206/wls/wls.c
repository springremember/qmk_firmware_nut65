...
bool hs_rgb_blink_hook() {
    ...
    uint32_t timeout = 0;
    static bool  hs_wlr_succeed_fg = false;
    ...
    switch (*md_getp_state()) {
        ...
        case MD_STATE_DISCONNECTED:{
            if (hs_rgb_blink_get_timer() == 0x00) {
                hs_rgb_blink_set_timer(timer_read32());
                if (hs_wlr_succeed_fg){
                    hs_wlr_succeed_fg = false;
        ...
        case MD_STATE_CONNECTED:{
            if (hs_rgb_blink_get_timer() == 0x00) {
                ...
                hs_wlr_succeed = true;
                hs_wlr_succeed_fg = true;
                hs_usb_succeed_time = timer_read32();
            } else {
                if (wireless_get_current_devs() == DEVS_USB) hs_rgb_blink_set_timer(timer_read32());
