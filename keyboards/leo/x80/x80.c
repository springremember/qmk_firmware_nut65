...
void hs_system_cut(uint8_t r,uint8_t g,uint8_t b);
...
static bool entre_batq_flag  = false;
    ...
    switch (keycode) {
        ...
        case SYS_TOG: {
            if (!keymap_is_mac_system()) {
                hs_system_cut(0xff,0xff,0xff);
                set_single_persistent_default_layer(_MBL);
                layer_move(0);
            } else {
                hs_system_cut(0x00,0x00,0xff);
...
bool process_record_kb(uint16_t keycode, keyrecord_t *record) {
    ...
    switch (keycode) {
        ...
        case HS_BATQ: {
            if (record->event.pressed) {
                entre_batq_flag = true;
            } else {
                entre_batq_flag = false;
            }
            return false;
        } break;
        default:
...
bool rgb_matrix_indicators_advanced_kb(uint8_t led_min, uint8_t led_max) {
    ...
    if (!rgbrec_is_started() && host_keyboard_led_state().scroll_lock) {
        rgb_matrix_set_color(HS_RGB_INDEX_SCR, RGB_WHITE);
    } else {
        rgb_matrix_set_color(HS_RGB_INDEX_SCR, RGB_BLACK);
    }

    if (host_keyboard_led_state().caps_lock) {
        rgb_matrix_set_color(HS_RGB_INDEX_CAPS_0, RGB_RED);
        rgb_matrix_set_color(HS_RGB_INDEX_CAPS_1, RGB_RED);
        rgb_matrix_set_color(HS_RGB_INDEX_CAPS_2, RGB_RED);
    } 

    rgb_matrix_set_color(HS_RGB_BLINK_INDEX_BT1, RGB_BLACK);
    rgb_matrix_set_color(HS_RGB_BLINK_INDEX_BT2, RGB_BLACK);
    rgb_matrix_set_color(HS_RGB_BLINK_INDEX_BT3, RGB_BLACK);
    rgb_matrix_set_color(HS_RGB_BLINK_INDEX_2G4, RGB_BLACK);
...
void query(void){
    if (entre_batq_flag) {
        ...
        if(*md_getp_bat() > 80  && *md_getp_bat() <= 100){
            rgb_matrix_set_color(HS_RGB_BLINK_INDEX_BT1, RGB_GREEN);
            rgb_matrix_set_color(HS_RGB_BLINK_INDEX_BT2, RGB_GREEN);
            rgb_matrix_set_color(HS_RGB_BLINK_INDEX_BT3, RGB_GREEN);
            rgb_matrix_set_color(HS_RGB_BLINK_INDEX_2G4, RGB_GREEN);
            rgb_matrix_set_color(HS_MATRIX_BLINK_INDEX_BAT, RGB_GREEN);
            ...
            rgb_matrix_set_color(HS_RGB_BLINK_INDEX_BT1, RGB_YELLOW);
            rgb_matrix_set_color(HS_RGB_BLINK_INDEX_BT2, RGB_YELLOW);
            rgb_matrix_set_color(HS_RGB_BLINK_INDEX_BT3, RGB_BLACK);
            rgb_matrix_set_color(HS_RGB_BLINK_INDEX_2G4, RGB_BLACK);
            rgb_matrix_set_color(HS_MATRIX_BLINK_INDEX_BAT, RGB_BLACK);
        }else if(*md_getp_bat() <= 20){
            rgb_matrix_set_color(HS_RGB_BLINK_INDEX_BT1, RGB_RED);
            rgb_matrix_set_color(HS_RGB_BLINK_INDEX_BT2, RGB_BLACK);
            rgb_matrix_set_color(HS_RGB_BLINK_INDEX_BT3, RGB_BLACK);
            rgb_matrix_set_color(HS_RGB_BLINK_INDEX_2G4, RGB_BLACK);
            rgb_matrix_set_color(HS_MATRIX_BLINK_INDEX_BAT, RGB_BLACK);
...
void hs_system_cut(uint8_t r,uint8_t g,uint8_t b) { 
    rgb_matrix_hs_set_remain_time(HS_RGB_INDEX_CAPS_0, 0);
    rgb_matrix_hs_indicator_set(HS_RGB_INDEX_CAPS_0, (RGB){r, g, b}, 250, 3);
    rgb_matrix_hs_set_remain_time(HS_RGB_INDEX_CAPS_1, 0);
    rgb_matrix_hs_indicator_set(HS_RGB_INDEX_CAPS_1, (RGB){r, g, b}, 250, 3);
    rgb_matrix_hs_set_remain_time(HS_RGB_INDEX_CAPS_2, 0);
    rgb_matrix_hs_indicator_set(HS_RGB_INDEX_CAPS_2, (RGB){r, g, b}, 250, 3);
}

void bat_indicators(void) {
    static uint32_t battery_process_time = 0;
    if (entre_batq_flag) return;
    ...
    if (charging_state && (bat_full_flag)) {
        ...
        battery_process_time = 0;
        rgb_matrix_set_color(HS_MATRIX_BLINK_INDEX_BAT, 0xff, 0, 0);
    } else if (*md_getp_bat() <= BATTERY_CAPACITY_LOW) {

        rgb_matrix_hs_bat_set(HS_MATRIX_BLINK_INDEX_BAT, (RGB){0x00, 0xff, 0x00}, 250, 1);
