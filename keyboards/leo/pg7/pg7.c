...
void hs_gui_lock(void);
void hs_rgb_blink(uint8_t hs_index,uint8_t r,uint8_t g,uint8_t b);
void hs_system_cut(uint8_t hs_index,uint8_t r,uint8_t g,uint8_t b);
...
static uint32_t hs_gui_timer = 0;
...
#    endif
    ...
    rgb_matrix_sethsv(start_hsv.h,start_hsv.s,0x00);
}
    ...
    switch (keycode) {
        ...
        case KC_USB: {
            if (record->event.pressed) {
                ...
                rgb_matrix_hs_set_remain_time(HS_RGB_BLINK_INDEX_BT1, 0);
                rgb_matrix_hs_set_remain_time(HS_RGB_BLINK_INDEX_BT2, 0);
                rgb_matrix_hs_set_remain_time(HS_RGB_BLINK_INDEX_BT3, 0);
                rgb_matrix_hs_set_remain_time(HS_RGB_BLINK_INDEX_2G4, 0);
                rgb_matrix_hs_set_remain_time(HS_RGB_BLINK_INDEX_USB, 0);
                rgb_matrix_hs_indicator_set(HS_RGB_BLINK_INDEX_USB, (RGB){0x00, 0xff, 0x00}, 250, 3);
            }
        } break;
        default:
            return true;
...
void wireless_devs_change_kb(uint8_t old_devs, uint8_t new_devs, bool reset) {
    ...
    switch (new_devs) {
        ...
        case DEVS_2G4: {
            if (reset) {
                rgb_matrix_wls_indicator_set(HS_RGB_BLINK_INDEX_2G4, (RGB){RGB_RED}, 200, 1);
            } else {
                rgb_matrix_wls_indicator_set(HS_RGB_BLINK_INDEX_2G4, (RGB){RGB_RED}, 500, 1);
            }
        } break;
        default:
            break;
...
bool rgb_matrix_indicators_advanced_kb(uint8_t led_min, uint8_t led_max) {
    ...
    if (hs_gui_timer && timer_elapsed32(hs_gui_timer) > 3000) {
        hs_gui_lock();
        hs_gui_timer = 0;
    }
    ...
    if ((*md_getp_state() == MD_STATE_CONNECTED) || USB_DRIVER.state == USB_ACTIVE)
    {
        if (!rgbrec_is_started() && host_keyboard_led_state().caps_lock){
            rgb_matrix_set_color(HS_RGB_INDEX_CAPS, 0xff, 0x00, 0x00);
        } else {
            rgb_matrix_set_color(HS_RGB_INDEX_CAPS, 0x00, 0x00, 0x00);
        }
    }
...
void hs_gui_lock(void) {
    mac_status = keymap_is_mac_system();
    if (!mac_status) {
        confinfo.hs_win_lock = !confinfo.hs_win_lock;
        if (confinfo.hs_win_lock) { 
            keymap_config.no_gui = 1;
            hs_system_cut(HS_RGB_SYSTEM_SWITCH_INDEX,0x00,0xff,0xff);
        } else { 
            keymap_config.no_gui = 0;
            hs_system_cut(HS_RGB_SYSTEM_SWITCH_INDEX,0xff,0x00,0x00);
        }
        eeconfig_confinfo_update(confinfo.raw);
    }
}
