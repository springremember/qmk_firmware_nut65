...
### 电源开关（仅无 USB 线时生效：`Delete` + `Space`）

| 操作 | 效果 |
| :--- | :--- |
| 运行中按住 `Delete`+`Space` ≥3s | 进入深睡（关闭/省电，近似关机） |
| 深睡中按住 `Delete`+`Space` ≥3s | 开机，恢复无线模式 |
| 深睡中按其它任意键 | 仅唤醒 MCU 瞬间，随即自动重回深睡（不会误开机） |
| 深睡中插入 USB 线 | 立即恢复，自动进入有线模式 |

> 有 USB 线时该组合键不生效；插线自动切换有线/无线仍为原厂行为。
