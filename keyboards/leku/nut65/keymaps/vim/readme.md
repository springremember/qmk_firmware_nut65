...
| 右 `Shift` + `←` / `→` | 音量减 / 音量增（可按住连调） |
| 右 `Shift` + `↑` / `↓` | 亮度增 / 亮度减（可按住连调） |
...
跟随模式色的键位灯（33% 亮度）：`Caps`、`Esc`、`Delete`（原 Insert 位）。
...
- `_BL`（win Base 层）按用户要求改动键位：`Insert→Delete`、原 `Delete` 位→**关闭程序**（`Alt+F4`；grave 由 右`Shift`+`Esc` 提供）、`PageUp→KC_WFWD`、`PageDown→KC_WBAK`，其余一致；右 `Shift` 组合键（grave/F 区/媒体键）见上文
