...
void bat_indicators(void) {
    ...
    static bool flashing_blue_flag       = false; 
    static bool flashing_green_flag      = false; 
    static uint32_t full_cnt             = 0; 

    if (charging_state && (bat_full_flag)) { 
        flashing_blue_flag = false;
        if (!flashing_green_flag) {
            if(timer_elapsed32(full_cnt) < 1500){
                for (uint8_t i = 117; i < 122; i++) {
                    rgb_matrix_hs_indicator_set(i, (RGB){0xFF, 0x00, 0x00}, 250, 1);
                }
            } else {
                flashing_green_flag = true;
            }
        }
        battery_process_time = 0;
    } else if (charging_state) { 
        flashing_green_flag = false;
        if (!flashing_blue_flag) {
            if(timer_elapsed32(full_cnt) < 1500){
                for (uint8_t i = 117; i < 122; i++) {
                    rgb_matrix_hs_indicator_set(i, (RGB){0x00, 0x00, 0xFF}, 250, 1);
                }
            } else {
                flashing_blue_flag = true;
            }
        }
        ...
        flashing_blue_flag  = false;
        flashing_green_flag = false;
        full_cnt = timer_read32();

        if (confinfo.use1_flag) {
            for (uint8_t i = 117; i < 122; i++) {
                rgb_matrix_hs_indicator_set(i, (RGB){0x00, 0xFF, 0x00}, 250, 1);
            }
        } 
...
void rgb_blink_dir(void) {
    ...
    rgb_matrix_hs_indicator_set(6, (RGB){0xFF, 0xFF, 0xFF}, 250, 3);
    rgb_matrix_hs_indicator_set(7, (RGB){0xFF, 0xFF, 0xFF}, 250, 3);
    rgb_matrix_hs_indicator_set(8, (RGB){0xFF, 0xFF, 0xFF}, 250, 3);
}
...
void rgb_matrix_start_rec(void) {
    ...
    if (rgbrec_is_started()) {
        ...
        if (rec_filp) {
            writePin(LED_CAPS_PIN, LED_PIN_ON_STATE);
            writePin(LED_WIN_PIN, LED_PIN_ON_STATE);
        } else {
            writePin(LED_CAPS_PIN, !LED_PIN_ON_STATE);
            writePin(LED_WIN_PIN, !LED_PIN_ON_STATE);
        }
