...
bool hs_modeio_detection(bool update, uint8_t *mode, uint8_t lsat_btdev) {
    ...
    extern bool hs_frequency_test;
    if(hs_frequency_test){
        if (wireless_get_current_devs() != DEVS_USB) wireless_devs_change(wireless_get_current_devs(), DEVS_USB, false); 
        return false;
    }
            ...
            if (hs_rgb_blink_get_timer() == 0x00) {
                ...
                if (!state){
                    if ((timer_elapsed32(hs_rgb_blink_get_timer()) >= HS_LBACK_TIMEOUT) && !rgbrec_is_started()) {
                        ...
                        wait_ms(200);
                        DEBUG("MD_STATE_DISCONNECTED LBACK\r\n");
                    ...
                    if ((timer_elapsed32(hs_rgb_blink_get_timer()) >= HS_PAIR_TIMEOUT) && !rgbrec_is_started()) {
                        ...
                        md_send_devctrl(MD_SND_CMD_DEVCTRL_USB);
                        wait_ms(200);
                        DEBUG("MD_STATE_DISCONNECTED PAIR\r\n");
                        lpwr_set_timeout_manual(true);
                    }
                }
            }
        ...
        case MD_STATE_CONNECTED:{
            ...
            if (hs_rgb_blink_get_timer() == 0x00) {
                ...
                if (timer_elapsed32(hs_rgb_blink_get_timer()) >= timeout && !rgbrec_is_started()) {
                    DEBUG("hs_rgb_blink_get_timer = %ld\r\n",timer_elapsed32(hs_rgb_blink_get_timer()));
                    hs_rgb_blink_set_timer(timer_read32());
                    // clear_keyboard();
                    DEBUG("MD_STATE_CONNECTED\r\n");
                    lpwr_set_timeout_manual(true);
                }
            }
        } break;
...
void lpwr_stop_hook_pre(void) { // SLEEP
    ...
    setPinInput(HS_ENCODERS_PAD_A);
#endif

#ifdef HS_ENCODERS_PAD_B
    setPinInput(HS_ENCODERS_PAD_B);
#endif
    DEBUG("lpwr_stop_hook_pre\r\n");
    state = false;
    if (lower_sleep) {
        md_send_devctrl(MD_SND_CMD_DEVCTRL_USB);
        wait_ms(200);
