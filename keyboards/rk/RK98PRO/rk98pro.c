...
void keyboard_post_init_kb(void) {
    ...
    if (1 && hs_get_rgb_enable()) gpio_write_pin_high(LED_POWER_EN_PIN);
    ...
    gpio_set_pin_output(LED_MAC);
    gpio_set_pin_output(LED_POV);
    ...
    switch (keycode) {
        ...
        case HS_DIR: {
            if (!keymap_is_mac_system()) {
                ...
                layer_move(0);
                rgb_matrix_hs_indicator_set(5,(RGB){0xFF, 0xFF, 0xFF}, 250, 3);
            }
            else{
                set_single_persistent_default_layer(_BL);
                layer_move(0);
                rgb_matrix_hs_indicator_set(5,(RGB){0x00, 0x00, 0xFF}, 250, 3);
            }
        } break;
        default:
            break;
    ...
    switch (keycode) {
        ...
        case KC_USB: {
            ...
            if ((mode == hs_2g4) || (mode == hs_wireless) || (mode == hs_none)) {
                ...
                rgb_matrix_hs_indicator_set(HS_RGB_BLINK_INDEX_USB, (RGB){0xFF, 0x00, 0x00}, 500, 3);
                state = false;
                hs_rgb_blink_set_timer(timer_read32());
            }
        } break;
...
bool rgb_matrix_wls_indicator_cb(void) {
    ...
    if (*md_getp_state() != MD_STATE_CONNECTED) {
        if (!(wireless_get_current_devs() == DEVS_USB)) wireless_devs_change_kb(wireless_get_current_devs(), wireless_get_current_devs(), wls_rgb_indicator_reset);
        return true;
    }
...
void bat_indicators(void) {
    ...
    if (charging_state && (bat_full_flag) && *md_getp_bat() > 90){
        ...
        battery_process_time = 0;
        writePin(LED_POV, 0);
        // if (!led_status && rgb_matrix_get_val()) rgb_matrix_set_color(HS_MATRIX_BLINK_INDEX_BAT, 0xFF, 0x00, 0x00);
        ...
        else rgb_matrix_set_color(HS_MATRIX_BLINK_INDEX_BAT, 0x00, 0xFF, 0x00);
...
void rgb_blink_dir(void) {
    rgb_blink = true;
    rgb_matrix_hs_indicator_set(0xF0, (RGB){0, 0, 0}, 250, 3);
}

bool hs_reset_settings_user(void) {

    rgb_matrix_hs_indicator_set(0xF0, (RGB){0x10, 0x00, 0x00}, 250, 3);
...
void nkr_indicators_hook(uint8_t index) {
    ...
    if ((hs_rgb_indicators[index].rgb.r == 0x00) && (hs_rgb_indicators[index].rgb.g == 0x6E) && (hs_rgb_indicators[index].rgb.b == 0x00)) {
        ...
        if (hs_rgb_indicators[i].active) {
            if (timer_elapsed32(hs_rgb_indicators[i].timer) >= hs_rgb_indicators[i].interval) {
                ...
                if (hs_rgb_indicators[i].times <= 0) {
                    ...
                    if (hs_rgb_indicators[i].index == 0xF0) {
                        if (rgbrec_is_started()) rgb_blink_dir();
                        else rgb_blink = false;
                    }
            ...
            if ((hs_rgb_indicators[i].times % 2)) {
                if (hs_rgb_indicators[i].index == 0xFF) {
                    rgb_matrix_set_color_all(hs_rgb_indicators[i].rgb.g, hs_rgb_indicators[i].rgb.r, hs_rgb_indicators[i].rgb.b);
                } else if (hs_rgb_indicators[i].index == 0xF0) {
                    writePin(LED_WIN, 1);
                    writePin(LED_CAPS_LOCK_PIN, 1);
                    writePin(LED_SCROLL_LOCK_PIN, 1);
                    writePin(LED_NUM_LOCK_PIN, 1);
                } else {
                    rgb_matrix_set_color(hs_rgb_indicators[i].index, hs_rgb_indicators[i].rgb.g, hs_rgb_indicators[i].rgb.r, hs_rgb_indicators[i].rgb.b);
                }
            } else {
                if (hs_rgb_indicators[i].index == 0xFF) {
                    rgb_matrix_set_color_all(0x00, 0x00, 0x00);
