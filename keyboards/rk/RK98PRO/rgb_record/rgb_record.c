    ...
    if (rk_bat_req_flag) {
        ...
        for (uint8_t i = 0; i < 5; i++) {
            ...
            if ((i < (*md_getp_bat() / 20)) || (i < 1)) {
                ...
                } else if (*md_getp_bat() >= (IM_BAT_REQ_LEVEL3_VAL))  {
                    rgb_matrix_set_color(mi_index[i], IM_BAT_REQ_LEVEL3_COLOR);
                }else if (*md_getp_bat() >= (IM_BAT_REQ_LEVEL4_VAL))  {
                    rgb_matrix_set_color(mi_index[i], IM_BAT_REQ_LEVEL4_COLOR);
                }else{
                    rgb_matrix_set_color(mi_index[i], IM_BAT_REQ_LEVEL5_COLOR);
                }
            } else {
